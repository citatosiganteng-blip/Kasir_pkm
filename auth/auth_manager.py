...
class AuthManager:
    ...
    def _normalize_role(role) -> str:
        """Normalisasi nilai role: buang spasi & samakan huruf kecil.
        Ini membuat pengecekan admin/kasir tetap benar walau nilai role
        di database ditulis tidak konsisten (mis. 'Admin', 'ADMIN', ' admin ')
        akibat penambahan akun manual lewat SQLite/DB Browser."""
        return (role or "").strip().lower()

    @property
    def is_admin(self) -> bool:
        return self._current_user is not None and self._normalize_role(self._current_user.role) == "admin"

    @property
    def is_kasir(self) -> bool:
        return self._current_user is not None and self._normalize_role(self._current_user.role) == "kasir"
