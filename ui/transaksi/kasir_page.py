...
class KasirPage(QWidget):
    ...
    def _on_payment_tab_toggled(self, btn: QPushButton, name: str, is_checked: bool):
        ...
        if is_checked:
            ...
            self.qris_frame.setVisible(name == "QRIS/E-Wallet")
            self.transfer_frame.setVisible(name == "Metode Lain")

            if name == "QRIS/E-Wallet":
                self._load_qris_image()
            elif name == "Metode Lain":
                self._load_transfer_info()

            self._update_checkout_button_label()

    def _load_qris_image(self):
        """Tampilkan gambar QRIS toko dari Pengaturan, sebelum transaksi diproses."""
        ...
        # Fallback jika belum ada gambar QRIS yang diupload di Pengaturan
        self.qris_img_lbl.setPixmap(QPixmap())
        self.qris_img_lbl.setText(
            "⚠️ Gambar QRIS toko belum diatur.\n\n"
            "Buka menu Pengaturan ⚙️ → Info Toko →\n"
            "Gambar QRIS Toko untuk mengunggahnya."
        )

    def _load_transfer_info(self):
        """Tampilkan info rekening bank toko dari Pengaturan, sebelum transaksi diproses."""
        bank_name = db.get_setting("store_bank_name", "")
        bank_acc = db.get_setting("store_bank_account", "")
        bank_holder = db.get_setting("store_bank_holder", "")

        if bank_acc:
            self.transfer_info_lbl.setText(
                f"Bank: <b>{bank_name or '-'}</b><br>"
                f"No. Rekening: <b>{bank_acc}</b><br>"
                f"Atas Nama: <b>{bank_holder or '-'}</b>"
            )
        else:
            self.transfer_info_lbl.setText(
                "⚠️ Nomor rekening toko belum diatur.\n\n"
                "Buka menu Pengaturan ⚙️ → Info Toko →\n"
                "Rekening Bank untuk mengisinya."
            )

    def _update_checkout_button_label(self):
        """Ubah label tombol checkout agar kasir memastikan pembayaran sudah
        diterima dulu untuk metode non-tunai, sebelum transaksi disimpan."""
        if self._selected_metode in ("QRIS/E-Wallet", "Metode Lain"):
            self.btn_checkout.setText("KONFIRMASI PEMBAYARAN DITERIMA")
        else:
    ...
    def _process_payment(self, bayar: float, metode: str):
        ...
        self.cart.clear()
        self._nominal_bayar = 0.0
        # Kembali ke metode "Tunai" -> otomatis menyembunyikan panel QRIS/transfer
        # dan mengembalikan label tombol lewat _on_payment_tab_toggled()
        if "Tunai" in self.pay_buttons:
            self.pay_buttons["Tunai"].setChecked(True)
        self._update_cart_display()
        self._load_barang()
