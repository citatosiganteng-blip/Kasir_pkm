...
class UserFormDialog(QDialog):
    ...
    def __init__(self, user=None, parent=None):
        ...
        # tebakan, supaya tidak ada label/field yang terpotong.
    ...
    def _setup_ui(self):
        ...
        self.password_input.setStyleSheet("border-top-right-radius: 0; border-bottom-right-radius: 0;")
        pass_row.addWidget(self.password_input)
        btn_toggle_pw = QPushButton()
        btn_toggle_pw.setIcon(eye_icon(False))
        btn_toggle_pw.setIconSize(QSize(16, 16))
        btn_toggle_pw.setFixedSize(40, 40)
        btn_toggle_pw.setCheckable(True)
        btn_toggle_pw.setCursor(Qt.PointingHandCursor)
        btn_toggle_pw.setObjectName("btn_secondary")
        btn_toggle_pw.toggled.connect(
            lambda chk, field=self.password_input, btn=btn_toggle_pw: (
                field.setEchoMode(QLineEdit.Normal if chk else QLineEdit.Password),
                btn.setIcon(eye_icon(chk)),
            )
        )
    ...
    def _populate(self):
        ...
        idx = self.role_combo.findData((u.role or "").strip().lower())
        if idx >= 0:
