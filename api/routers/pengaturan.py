...
@pengaturan_router.get("/qris/image")
def get_qris_image(_: dict = Depends(get_current_user_payload)):
    """
    Sajikan file gambar QRIS toko lewat HTTP agar bisa ditampilkan di
    web/mobile client (Android/tablet), bukan cuma path lokal di PC kasir.
    Diletakkan SEBELUM route '/{kunci}' agar tidak tertangkap sebagai kunci pengaturan.
    """
    qris_path = db.get_setting("qris_image_path", "")
    if not qris_path or not _Path(qris_path).exists():
        raise HTTPException(status_code=404, detail="Gambar QRIS toko belum diatur")
    return FileResponse(qris_path)
